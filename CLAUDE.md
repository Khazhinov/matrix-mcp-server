...
- **`backupRestore.ts`** - built on `@matrix-org/matrix-sdk-crypto-wasm` (the binding `matrix-js-sdk`/Element use), invoked only as a fallback when the primary engine can't decrypt an event. Restores exactly the one needed session from the account's server-side key backup (via `MATRIX_RECOVERY_KEY`) into a throwaway, **in-memory-only** WASM `OlmMachine` (no `store_name` passed - confirmed to avoid touching IndexedDB entirely, which doesn't exist in Node anyway), decrypts, and discards the instance. Consults `sessionCache.ts` first (see below) before repeating the network round trip for a session it's already restored.
- **`sessionCache.ts`** - a small, self-owned SQLite database (Node's built-in `node:sqlite`, not related to either crypto engine's own storage) caching already-decrypted session-export data keyed by `(roomId, sessionId)`. One megolm session covers many messages; without this, `backupRestore.ts` would repeat its full network+decrypt round trip per *event* instead of per *session*.
...
**`src/matrix/historyPagination.ts` is not E2EE-specific but is load-bearing for history restore to mean anything**: `get-room-messages`/`get-messages-by-date` only ever read `room.getLiveTimeline().getEvents()`, which never reaches further back than the initial sync + whatever arrived live on its own. `ensureMinimumEventCount`/`ensureHistoryBefore` call `client.scrollback()` as needed before either tool handler reads the timeline (capped at 20 iterations). Confirmed this gap exists identically on unencrypted rooms - it is a general pagination limitation, not a crypto one, but without it the key-backup-restore fallback above is unreachable for anything older than what happened to already be loaded.
